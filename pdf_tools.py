...
def merge_with_affine_scaling(uploaded_files_dict, page_order_list, target_size_setup, out_path):
    ...
    for file_id, file_path in uploaded_files_dict.items():
        ...
        if reader.is_encrypted:
            try: reader.decrypt("")
            except: raise Exception(f"File {file_id} is locked.")
        ...
        if file_id in readers:
            ...
            if "/Annots" in orig_page:
                for annot in orig_page["/Annots"]:
                    ...
                    if obj and "/T" in obj: obj.update({"/T": f"{obj['/T']}_dv_{field_collision_counter}"})
...
def split_pdf_advanced_core(in_path, mode, parameter_str, selected_indices, out_path_target, is_zip_request=True):
    ...
    if reader.is_encrypted:
        try: reader.decrypt("")
        except: raise Exception("File encrypted.")
    ...
    if mode == "visual":
        ...
        chunks_to_package.append(("split_document.pdf", writer))
    elif mode == "burst":
        ...
        for i in range(0, total_pages, chunk_size):
            ...
            chunks_to_package.append((f"chunk_{i+1}_to_{end_range}.pdf", writer))
    elif mode == "range":
        ...
        chunks_to_package.append(("range_document.pdf", writer))
